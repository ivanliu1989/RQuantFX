...
configs = list(
    order.book.file = '~/Common/audcad_intraday_m60/orderbook/AUDCAD_OANDA_H1.csv' # Order book location
    ,activity.book = '~/Common/audcad_intraday_m60/orderbook/AUDCAD_OANDA_H1_ACTIVITY.csv'
    ,log.file = file(paste0('~/Common/audcad_intraday_m60/log/AUDCAD_OANDA_H1_', as.Date(Sys.time()), '.Rout'), open = 'wt') # Log file location
    ,plotPath = "~/Common/audcad_intraday_m60/plots/audcad_intraday_H1_" # ZScore plots location
    ,modelPath = "~/Common/audcad_intraday_m60/models/audcad_intraday_H1_"
    ,strategyPath = '~/Common/audcad_intraday_m60/' # Strategy root folder
    ,pair1 = c('AUD', 'CAD') # Currency Pair
    ,initAssets = 10000 # net cap
    ,leverage = 20 # leverage
    ,reinvest = TRUE # reinvest or not
    ,posiRate = 0.002 # position ratio
    ...
    tryCatch({
        ...
        if(latestPrices$update.date > lastDate){
            ...
            if(ml.direction == zscore.direction &
               ...
               (abs(zScore) >= configs$threshold | aud.ml.ret <= xgbFitClass$pred.quantile['30%'] | aud.ml.ret >= xgbFitClass$pred.quantile['70%'])){
            ...
            if(length(openPos)>0){
                for(i in 1:nrow(openPos)){
                    ...
                    }else if((clsPrice-as.numeric(t$price))/(as.numeric(t$takeProfitOrder.price)-as.numeric(t$price))>=0.7){
                        clsRes = cloaseOandaTrade(configs$ACCOUNT_TYPE, configs$ACCESS_TOKEN, configs$ACCOUNT_ID, t$id)
                        tradeActivity = data.frame(as.character(Sys.time()), OB.PRICES, -as.numeric(t$currentUnits),
                                                   OB.PRICES,as.character(Sys.time()), 0, 0, 'Close', 0)
                        setDT(tradeActivity)
