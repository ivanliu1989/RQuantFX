...
configs = list(
    order.book.file = '~/Common/AUDCAD_OANDA_ML_H1_Paper/orderbook/AUDCAD_OANDA_H1.csv' # Order book location
    ,activity.book = '~/Common/AUDCAD_OANDA_ML_H1_Paper/orderbook/AUDCAD_OANDA_H1_ACTIVITY.csv'
    ,log.file = file(paste0('~/Common/AUDCAD_OANDA_ML_H1_Paper/log/AUDCAD_OANDA_H1_', as.Date(Sys.time()), '.Rout'), open = 'wt') # Log file location
    ,plotPath = "~/Common/AUDCAD_OANDA_ML_H1_Paper/plots/audcad_intraday_H1_" # ZScore plots location
    ,modelPath = "~/Common/AUDCAD_OANDA_ML_H1_Paper/models/audcad_intraday_H1_"
    ,strategyPath = '~/Common/AUDCAD_OANDA_ML_H1_Paper/' # Strategy root folder
    ,pair1 = c('AUD', 'CAD') # Currency Pair
    ,initAssets = 100000 # net cap
    ,leverage = 20 # leverage
    ,reinvest = TRUE # reinvest or not
    ,posiRate = 0.02 # position ratio
    ...
    ,ACCOUNT_TYPE_PRICE = 'practice' # Broker account type
    ,ACCOUNT_ID = '' # Broker account number
    ,train.length = 680 # Mean reversion lookback
    ,ml.length = 4800 # Machine learning lookback
    ,backtest.n = 4800 # Backtest periods
    ,min.lookback = 48 # Min half life
    ,max.lookback = 120 # Max half life
    ,sd.n = 3 # Standard deviation for SL & TP
    ,freq = 'M15' # Trading interval
    ,secondary.freq = "M5" # Secondary frequency to measure short term movements
    ,pred.n = 4 # Machine learning look ahead
...
AUDCAD = prepareForexOandaPrices(configs$ACCOUNT_TYPE_PRICE, configs$ACCESS_TOKEN_PRICE,
                                 oanda.count = 5000, Cur1 = 'AUD', Cur2 = 'CAD', oanda.granularity = configs$freq)
...
while(TRUE){
    ...
    tryCatch({
        ...
        if(latestPrices$update.date > lastDate | !exists("xgbFitClass")){
            ...
            if(!exists("xgbFitClass")){
                ...
                xgbFitClass1 = xgbBackTest(na.omit(merge(price.feat[1:2400,], oa.ret)))
                xgbFitClass2 = xgbBackTest(na.omit(merge(price.feat[2401:nrow(price.feat),], oa.ret)))
                price.feat$lastPred = c(predict(xgbFitClass1$xgbFit, data.matrix(price.feat[2401:nrow(price.feat),])),
                                        predict(xgbFitClass2$xgbFit, data.matrix(price.feat[1:2400,])))
            ...
            # 7. Risk Management ------------------------------------------------------
            AUD.sd = max(2 * configs$sd.n * sd(tail(cur.spreads, look.back)), 0.002)# )mean(sd(tail(cur.spreads, 12)), 
            ...
            if(aud.ml.ret < xgbFitClass$pred.quantile['10%']){
                # goLong = FALSE
                # goShort = TRUE # buy
                # closePos = FALSE
                goLong = TRUE # sell
                goShort = FALSE
                closePos = FALSE
            }else if(aud.ml.ret > xgbFitClass$pred.quantile['90%']){
                # goLong = TRUE # sell
                # goShort = FALSE
                # closePos = FALSE
                goLong = FALSE
            ...
            if(length(openPos)>0){
                for(i in 1:nrow(openPos)){
                    ...
                    if(hrDiff >= configs$pred.n/4 & as.numeric(t$unrealizedPL) > 0){
                        # Close positions
...
sink(type="message")
sink()
# ecoEvent = EconomicCalendarOanda(configs$ACCOUNT_TYPE, configs$ACCESS_TOKEN, configs$ACCOUNT_ID, INSTRUMENTS = 'AUD_CAD', PERIOD = '1 week')
# ecoEvent = EconomicCalendarOanda(configs$ACCOUNT_TYPE, configs$ACCESS_TOKEN, configs$ACCOUNT_ID, INSTRUMENTS = 'AUD_USD', PERIOD = '1 week')
# ecoEvent = EconomicCalendarOanda(configs$ACCOUNT_TYPE, configs$ACCESS_TOKEN, configs$ACCOUNT_ID, PERIOD = '1 week')
# ecoEvent$timestamp

# getAutoChartistSignal(configs)
